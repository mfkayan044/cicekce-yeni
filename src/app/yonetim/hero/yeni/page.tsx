...
export default function YeniHeroPage() {
  const router = useRouter();
  const [title, setTitle] = useState("");
  const [subtitle, setSubtitle] = useState("");
  const [badge, setBadge] = useState("");
  const [btnText, setBtnText] = useState("Hemen İncele");
  const [link, setLink] = useState("/kategori/gul-buketleri");
  const [image, setImage] = useState("https://demo.procicek.com.tr/resimler/banner-20260721163450.webp");
      ...
      <div className="space-y-6 max-w-4xl">
        <div className="flex justify-between items-center bg-white p-4 rounded-2xl border shadow-xs">
          <div className="flex items-center gap-3">
            ...
            <h4 className="font-extrabold text-xl text-slate-800 m-0">Yeni Hero Slayt Ekle</h4>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="card border-0 shadow-sm rounded-2xl p-6 bg-white space-y-4">
          <div>
            ...
            <input
              ...
              placeholder="Örn: Bahara Özel Taze Gül Buketleri"
              value={title}
          ...
          <div>
            ...
            <input
              type="text"
              className="w-full p-3 border border-slate-200 rounded-xl text-sm font-semibold outline-none focus:border-[#2b2623]"
              placeholder="Örn: Aynı Gün Ücretsiz Kurye Teslimatı"
              value={subtitle}
          ...
          <div>
            ...
            <input
              type="text"
              className="w-full p-3 border border-slate-200 rounded-xl text-sm font-semibold outline-none focus:border-[#2b2623]"
              placeholder="/kategori/gul-buketleri"
              value={link}
          ...
          <div>
            ...
            <input
              type="text"
              className="w-full p-3 border border-slate-200 rounded-xl text-sm font-semibold outline-none focus:border-[#2b2623]"
              placeholder="https://demo.procicek.com.tr/resimler/banner-20260721163450.webp"
              value={image}
          ...
          <div className="pt-4 flex justify-end gap-3">
            ...
            >
              {saving ? "Kaydediliyor..." : "Supabase'e Kaydet"}
            </button>
          </div>
        </form>
      </div>

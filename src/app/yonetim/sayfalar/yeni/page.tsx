...
export default function GenericFormPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    setSaved(true);
    setTimeout(() => router.push("/yonetim/sayfalar"), 1000);
  };
      ...
      <div className="space-y-6 max-w-4xl">
        <div className="flex justify-between items-center bg-white p-4 rounded-xl border shadow-sm">
          <div className="flex items-center gap-3">
            <Link href="/yonetim/sayfalar" className="btn btn-outline-secondary btn-sm flex items-center gap-1 rounded-lg">
              <i className="bx bx-left-arrow-alt text-lg"></i>
              <span>Geri / İptal</span>
            </Link>
            <h4 className="font-bold text-xl text-slate-800 m-0">Yeni Kurumsal Sayfa Ekle</h4>
          </div>
        ...
        <form onSubmit={handleSubmit} className="card border-0 shadow-sm rounded-xl p-6 bg-white space-y-4">
          ...
          <div>
            <label className="text-xs font-semibold text-slate-700 block mb-1">Sayfa Başlığı *</label>
            <input type="text" className="w-full px-3 py-2 border rounded-lg text-sm" placeholder="Gizlilik Politikası" required />
          </div>
          <div>
            <label className="text-xs font-semibold text-slate-700 block mb-1">URL (slug)</label>
            <input type="text" className="w-full px-3 py-2 border rounded-lg text-sm" placeholder="/gizlilik-politikasi" required />
          </div>
          <div>

...
export async function GET() {
  try {
    ...
    const settings = (data && data.value) ? data.value : (readDb().generalSettings || {});
...
export async function POST(request: Request) {
  try {
    ...
    const db = readDb();
    const existing = db.generalSettings || {};
    const updated = { ...existing, ...body };

    await supabase
      .from("site_settings")
      .upsert({ id: "general_settings", value: updated, updated_at: new Date().toISOString() });
